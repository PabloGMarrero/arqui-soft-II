...
sequenceDiagram
    ...
    Usuario->>Controller: GET /products/byName/{name}
    Controller->>UseCase: get
    UseCase->>GetProductAdapter: Obtener productos por nombre
    GetProductAdapter->>MongoGetProduct: Obtener productos por nombre
    MongoGetProduct->>DB: Obtener products por nombre
